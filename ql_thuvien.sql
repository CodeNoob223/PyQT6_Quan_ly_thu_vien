...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th7 02, 2024 lúc 06:32 PM
...
CREATE TABLE `admin` (
  ...
  `ROLE` varchar(30) NOT NULL DEFAULT 'THU_THU',
...
CREATE TABLE `sach` (
  ...
  `TOMTAT` varchar(400) NOT NULL,
  `ANHSACH` varchar(255) NOT NULL,
  `NGAYTHEM` date NOT NULL DEFAULT current_timestamp(),
...
(1, 3, 2, 'Anh chàng hiệp sĩ gỗ', 89, 'Dãy C', 'Ở thị trấn Bến Cam, mỗi năm cứ đến ngày gần Tết người ta lại thấy ông lão già ấy. Không ai biết quê quán ông lão ở đâu, họ tên ông lão là gì. Nhưng mỗi năm vào dịp Tết người ta lại thấy ông lão đẩy cái xe bánh gỗ lọc khọc đến, ăn mấy phiên chợ Tết...', 'D:/Designer QLTV/anhSach/anhchanghiepsigo.jpg', '2023-12-28', '2024-06-15'),
...
(11, 6, 12, 'Truyện Kiều', 94, 'Dãy C', 'Tai họa đã đột ngột ập đến Vương gia trong lúc người thiếu nữ còn đang thổn thức với mối tình đầu. Thằng bán tơ đã lén chôn một chai rượu vào vườn nhà Kiều rồi vu oan cho Vương ông tội buôn lậu rượu. Ngay lập tức, bọn sai nha xông vào, treo ngược Vương ôn', 'D:/Designer QLTV/anhSach/truyenkieu.jpg', '2024-04-23', '2024-06-13');

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `sach_muon`
--
...
CREATE TABLE `sach_muon` (
  ...
  `TINHTRANG` varchar(100) NOT NULL DEFAULT 'Chưa trả'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- Đang đổ dữ liệu cho bảng `sach_muon`
--

INSERT INTO `sach_muon` (`MASACH`, `MATHEMUON`, `NGAYMUON`, `NGAYTRA`, `TINHTRANG`) VALUES
(1, 1, '2023-12-29', '2024-06-18', 'Chưa trả'),
...
(1, 11, '2024-06-13', '2024-06-18', 'Chưa trả'),
...
(11, 31, '2024-06-19', '2024-07-20', 'Chưa trả'),
...
--
-- Cấu trúc bảng cho bảng `themuontra`
--
...
(1, 1, 2, 'Làm hỏng sách', '2023-12-29'),
(2, 1, 3, 'Làm mất sách', '2023-12-29'),
(3, 1, 2, 'Xả rác trong thư viện', '2024-04-26'),
(5, 3, 3, 'Làm mất sách', '2024-06-08');
...
--
-- AUTO_INCREMENT cho bảng `themuontra`
--
ALTER TABLE `themuontra`
...
--
-- Các ràng buộc cho bảng `sach_muon`
--
ALTER TABLE `sach_muon`
...
--
-- Các ràng buộc cho bảng `themuontra`
--
ALTER TABLE `themuontra`
